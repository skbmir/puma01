...
public:
    ...
    void init(ros::NodeHandle &nh, char * tty, DWORD mode, void * can_baudrate) 
    {
        ...
        tty_ = new char[sizeof(tty)];
        tty_ = strcpy(tty_, tty);
    ...
    // open CAN port
        ...
        if(!usbcan_handle_.open(tty,mode,can_baudrate))
        {
            ROS_ERROR_STREAM("Failed to connect to USB-CAN adapter and open port! Status: " << usbcan_handle_.getStatusString());
        }else{
            ROS_INFO_STREAM("Successfuly connected to USB-CAN adapter and opened port! Status: " << usbcan_handle_.getStatusString());
        }
       ...
       if(usbcan_handle_.noError())
        {
            if(usbcan_handle_.readRequest(read_buffer_.data(),read_buffer_.size())) // read request
            {
                ...
                ROS_ERROR_STREAM("Failed to READ data from USB-CAN adapter. Status: " << usbcan_handle_.getStatusString());
            }
            ...
            ROS_ERROR_STREAM("Error detected: " << usbcan_handle_.getStatusString());
...
int main(int argc, char **argv)
{
    ...
    usbcan_test_write_inst.init(nh, tty, VSCAN_MODE_NORMAL, VSCAN_SPEED_500K);
